/*
 *
 * Copyright (C) 2024-05-06 16:12 dongbin <dongbin0625@163.com>
 *
 */
#include <MQTTAsync.h>
...
int msgarrvd(void *context, char *topicName, int topicLen, MQTTAsync_message *message) {
    ...
    //   json_error_t error;
    // mqtt_class->root = json_loads((char *) message->payload, 1, &error);
    // if (wget_url(mqtt_class) != 0) {
    //     perror("url error");
    //     return -1;
    // }
...
int main() {
    ...
    printf("client_id: %s\n", mqtt_class->client_id);
    // Mqtt_class_t *mqtt_class = (Mqtt_class_t *) arg;
    ...
    if ((rc = MQTTAsync_setCallbacks(mqtt_class->client, mqtt_class->client, connlost, msgarrvd, NULL)) !=
        MQTTASYNC_SUCCESS) {
        // goto destroy_exit;
        printf("MQTTAsync_setCallbacks error\n");
    }
    printf("3\n");
    ...
    if ((rc = MQTTAsync_connect(mqtt_class->client, &conn_opts)) != MQTTASYNC_SUCCESS) {
        // goto destroy_exit;
        printf("MQTTAsync_Connected error\n");
    }
    printf("4\n");
    do {
        ch = getchar();
